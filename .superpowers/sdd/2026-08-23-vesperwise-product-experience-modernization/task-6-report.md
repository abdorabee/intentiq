...
- Provider, coverage/unscorable, credit, persistence, and completion failures
  retain the entered domain and expose Retry, Change domain, and Skip for now.
...
- The endpoint accepts a strict draft shape, enforces legal step progression,
  scopes every upsert to the Clerk user, reads the changed row back, and rejects
  zero-row or cross-user results.
...
- Added migration `20260823181538_onboarding_completion_version.sql`, including
  a nonnegative version constraint and legacy completed-user backfill.
...
```text
npx vitest run <9 Task 6 focused files>
Test Files  9 passed (9)
Tests  49 passed (49)
```

The completion route now contains nine focused cases, including authentication,
...
npm test
Test Files  75 passed | 5 skipped (80)
Tests  394 passed | 22 skipped (416)
