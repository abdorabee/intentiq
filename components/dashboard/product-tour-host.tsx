...
export function ProductTourHost({
  ...
  const [position, setPosition] = useState({ placement: "center" as TourPlacement, top: 16, left: 16, maxWidth: 360 });
  ...
  const visible = initialValidationRef.current.success
    ...
    && state.progress.tour_status === "in_progress";
  const step = PRODUCT_TOUR_STEPS[state.progress.tour_step] ?? PRODUCT_TOUR_STEPS[0];
  const onStepRoute = visible && pathname === step.route;
  ...
  useEffect(() => {
    if (!initialValidationRef.current.success || activeVersion <= 0 || autoStartRef.current) return;
    ...
    void perform("start");
  }, [activeVersion, perform, state.progress]);

  useEffect(() => {
    if (!visible || state.saving || pathname === step.route) return;
    router.push(step.route);
  }, [pathname, router, state.saving, step.route, visible]);
  ...
  useLayoutEffect(() => {
    ...
    return () => {
      restoreTourFocus(previousFocusRef.current);
      previousFocusRef.current = null;
    };
  }, [mounted, onStepRoute, state.progress.tour_step, targetReady]);
  ...
  useEffect(() => {
    ...
    function onKeyDown(event: KeyboardEvent) {
      ...
      } else if (event.key === "ArrowLeft" && state.progress.tour_step > 0) {
        event.preventDefault();
        void perform("back");
      } else if (event.key === "ArrowRight") {
        event.preventDefault();
        void perform(state.progress.tour_step === PRODUCT_TOUR_STEPS.length - 1 ? "finish" : "next");
      }
    }
    document.addEventListener("keydown", onKeyDown, true);
    return () => document.removeEventListener("keydown", onKeyDown, true);
  }, [onStepRoute, perform, state.progress.tour_step, state.saving]);
  ...
  return createPortal(
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="font-mono text-[11px] uppercase tracking-[0.16em] text-slate-500">{state.progress.tour_step + 1} of {PRODUCT_TOUR_STEPS.length}</p>
      ...
      <p className="sr-only" aria-live="polite">Step {state.progress.tour_step + 1} of {PRODUCT_TOUR_STEPS.length}: {step.title}</p>
      <div className="mt-5 flex flex-wrap items-center gap-2">
        ...
        {state.progress.tour_step > 0 && (
          <Button type="button" variant="outline" onClick={() => void perform("back")} disabled={state.saving}>
            <ArrowLeft className="size-4" aria-hidden /> Back
          </Button>
        )}
        <Button type="button" onClick={() => void perform(state.progress.tour_step === PRODUCT_TOUR_STEPS.length - 1 ? "finish" : "next")} disabled={state.saving}>
          {state.progress.tour_step === PRODUCT_TOUR_STEPS.length - 1 ? "Finish" : "Next"}
          {state.progress.tour_step < PRODUCT_TOUR_STEPS.length - 1 && <ArrowRight className="size-4" aria-hidden />}
        </Button>
      </div>
    </section>,
    document.body,
