          ...
          {DISPLAY_PLANS.map((p) => (
            <div
              ...
              className={`flex flex-col rounded-xl border p-5 ${
                p.highlight
                  ? "border-[#5e6ad2]/40 bg-gradient-to-b from-[#5e6ad2]/10 to-[#0e1011] shadow-[0_0_40px_rgba(94,106,210,0.15)]"
                  : "border-white/[0.08] bg-[#0e1011]"
              }`}
            >
              {p.highlight && (
                <span className="mb-3 w-fit rounded-full bg-[#5e6ad2] px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-white">
                  Most popular
                </span>
              )}
              ...
              <ul className="mt-4 flex-1 space-y-2 border-t border-white/[0.06] pt-4">
                {p.features.map((f) => (
                  <li key={f} className="flex items-start gap-2 text-[13px] text-[#8a8f98]">
                    <Check className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[#7170ff]" aria-hidden />
                    {f}
                  </li>
                ))}
              </ul>
              <Link
                ...
                className={`mt-5 inline-flex items-center justify-center rounded-full px-4 py-2.5 text-[14px] font-medium transition-[filter,box-shadow] ${
                  p.highlight
                    ? "bg-gradient-to-br from-[#8b87ff] via-[#5e6ad2] to-[#4e5acb] text-white shadow-[0_8px_28px_rgba(94,106,210,0.35)] hover:brightness-110"
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-white/[0.04]">
                {COMPARISON.map((c) => (
                  <tr key={c.name} className={c.you ? "bg-[#5e6ad2]/8" : ""}>
                    <td className="px-4 py-3 text-[#b4bbc8]">
                      {c.you ? <span className="font-semibold text-[#c9c4ff]">{c.name}</span> : c.name}
                    </td>
                    <td className={`px-4 py-3 ${c.you ? "font-medium text-[#c9c4ff]" : "text-[#8a8f98]"}`}>{c.price}</td>
                    {(["smb", "api", "ai", "mena"] as const).map((k) => (
                      <td key={k} className="px-4 py-3">
                        {c[k] ? (
                          <Check className="h-4 w-4 text-[#7170ff]" aria-label="yes" />
                        ) : (
