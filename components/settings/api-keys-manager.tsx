    ...
    <div className="space-y-6">
          ...
          <ul className="divide-y divide-slate-200 dark:divide-white/10">
            {payload.keys.map((key) => (
              <li key={key.id} className="flex flex-col gap-3 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                {key.is_active ? (
                  <button type="button" aria-label={`Revoke ${key.label}`} onClick={() => setRevokeTarget(key)} className="border border-red-500/30 px-3 py-2 text-xs text-red-600 hover:bg-red-500/5 dark:text-red-400">Revoke</button>
      ...
      {revokeTarget && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4" role="presentation">
          <div role="dialog" aria-modal="true" aria-labelledby="revoke-title" className="w-full max-w-md border border-slate-200 bg-white p-5 shadow-2xl dark:border-white/10 dark:bg-slate-950">
            ...
            <div className="mt-5 flex justify-end gap-2">
              <button type="button" disabled={revoking} onClick={() => setRevokeTarget(null)} className="border border-slate-300 px-3 py-2 text-sm dark:border-white/20">Cancel</button>
              <button type="button" disabled={revoking} onClick={() => void confirmRevoke()} className="inline-flex items-center gap-2 bg-red-600 px-3 py-2 text-sm font-medium text-white disabled:opacity-50">
